...
Por otro parte, también podemos agrupar nuestros datos dependiendo de si reciben algún tipo de ayuda en el estudio, por ejemplo: mediante clases particulares, ayuda familiar o soporte extra en el colegio. 
...
En esta sección, vamos a intentar responder a algunas de las preguntas que se plantearon en los objetivos iniciales mediante distintas pruebas de hipótesis. Como si las chicas 
...
```{r message= TRUE, warning=FALSE}
t.test(students.nopaid$score, students.paid$score, alternative = "less")
```

```{r message= TRUE, warning=FALSE}
t.test(students.noschoolsup$score, students.noschoolsup$score, alternative = "less")
```

```{r message= TRUE, warning=FALSE}
t.test(students.nofamsup$score, students.famsup$score, alternative = "less")
```
